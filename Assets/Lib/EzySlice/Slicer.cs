        ...
        public static SlicedHull Slice(Mesh sharedMesh, Plane pl, TextureRegion region, int crossIndex)
        {
            ...
            for (int i = 0; i < slices.Length; i++)
            {
                if (slices[i] != null)
                {
                    ...
                    if (slices[i].isValid)
                    //if (slices[i] != null && slices[i].CustomValid)
                    {
                        return CreateFrom(slices, CreateFrom(crossHull, pl.normal, region), crossIndex);
                    }
                    else
                    {
                        Debug.LogWarningFormat("slices[{0}] isn't valid. LowerHull: {1}; UpperHull: {2};", i,slices[i].lowerHull.Count,slices[i].upperHull.Count);
                    }
                }
                else
                {
                    Debug.LogWarningFormat("slides[{0}] is null.",i);
                }
            }
